...
export default function ManageProducts() {
  ...
  const categoryOptions = Array.from(
    new Map(
      productData
        ...
        .map((cat) => [cat.CategoryID, cat.CategoryName])
    ).entries()
  ).map(([id, name]) => ({ CategoryID: id, CategoryName: name }));
  ...
  const filteredData = productData.filter((item) => {
    ...
    const matchesCategory =
      !selectedCategory ||
      item.ProductCategories.some((cat) => cat.CategoryID.toString() === selectedCategory);

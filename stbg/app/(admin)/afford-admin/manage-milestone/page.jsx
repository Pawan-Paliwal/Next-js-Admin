...
export default function ManageMilestoneData() {
  ...
  const [updateDisplayOrder] = useUpdateMilestoneDisplayOrderMutation();
  ...
  useEffect(() => {
    if (!Array.isArray(milestoneData) || milestoneData.length === 0) return;
    setDisplayOrders(prev => {
      const next = { ...prev };
      let changed = false;
      milestoneData.forEach(m => {
        if (next[m.MilestoneID] !== m.DisplayOrder) {
          next[m.MilestoneID] = m.DisplayOrder;
          changed = true;
        }
      });
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    const confirmed = confirm("Are you sure you want to update display order?");
    if (!confirmed) return;

...
export default function AddUpdUser() {
  ...
  // Generate unique password using name + random string + year
  const generatePassword = () => {
    const name = formData.FullName.trim().replace(/\s+/g, "");
    if (!name) return "";

    // Get first 4 chars of name (or less if name is shorter)
    ...
    // Combine: name prefix + random number + year + special char
    return `${namePrefix}${randomNum}${year}@`;
  };
  ...
  const handleSubmit = async () => {
    ...
    formDataToSend.append(
      "UserName",
      formData.FullName
        ? `${formData.FullName.trim().replace(/\s+/g, ".").toLowerCase()}.${formData.PhoneNumber.slice(-4)}.afford`
        : ""
    );
    ...
    <main className="add_update container">
      <div className="form-box">
        ...
        <div >
          ...
          <Link href="/afford-admin/manage-user" className="back-btn">
            Back
          </Link>
        </div>
      </div>

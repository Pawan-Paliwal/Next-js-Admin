...
export default function ManageTestimonialData() {
  ...
  const [testimonialTypes, setTestimonialTypes] = useState([]);
  ...
  useEffect(() => {
    if (Array.isArray(testimonials)) {
      ...
      testimonials.forEach(t => {
        (t.TestimonialPages || []).forEach(p => pagesSet.add(p));
      });
      setTestimonialTypes([...pagesSet]);
    }
  }, [testimonials]);
  ...
  const filteredData = testimonials.filter((item) => {
    ...
    const matchesType = !selectedType || (item.TestimonialPages || []).includes(selectedType);

...
import { useLogoutMutation, authAPISlice, useUpdatePasswordMutation, useCheckLoginQuery } from "../../store/backendSlice/authAPISlice";
import { useDispatch } from "react-redux";
import { clearAuthData } from "../../store/backendSlice/authReducer";
...
export default function SideNav() {
  ...
  useEffect(() => {
    ...
    if (storedUser?.loginID === 1 || storedUser?.UserType === "SUPERADMIN") {
      filtered = Menu;
    } else {
      Menu.forEach((item) => {
        if (item.PageID === 1) {
            ...
            <ul className="Header_nav_Active">
              {allowedMenu
                ...
                .map((item, index) => {
                  ...
                  let isActive = false;
                  if (from) {
                    isActive = menuKey === from;
                  } else {
                    const isDirect = pathname === item.url || pathname === item.addurl;
                    const isSub = item.MoreItem?.some(sub => pathname === sub.url || pathname === sub.addurl);
                    isActive = isDirect || isSub;
                  }
                              ...
                              {subItem.title === "Update Sitemap" ? (
                                <Link
                                  ...
                                  onClick={async (e) => {
                                    ...
                                    e.stopPropagation();
                                    try {
                                      toast.loading("Saving sitemap...", { id: "sitemap" });
                                      const response = await fetch("/api/sitemap?action=save");
                                      const result = await response.json();

                                      if (result.success) {
                                        toast.success("Sitemap saved to sitemap.xml", { id: "sitemap" });
                                        setTimeout(() => {
                                          window.open("/sitemap.xml", "_blank");
                                        }, 500);
                                      } else {
                                        toast.error(result.message || "Failed to save sitemap", { id: "sitemap" });
                                      }
                                    } catch (error) {
                                      toast.error("Error saving sitemap", { id: "sitemap" });
              ...
              {(checkData?.user?.Role === "Super Admin") && (
                <>
                  <li>
              ...
              <li>
                <a onClick={handleLogout} style={{ cursor: "pointer" }}>
                  <svg xmlns='http://www.w3.org/2000/svg' xmlnsXlink='http://www.w3.org/1999/xlink' aria-hidden='true' role='img' className='iconify iconify--hugeicons' width='1em' height='1em' preserveAspectRatio='xMidYMid meet' viewBox='0 0 24 24' data-icon='hugeicons:logout-04'>
                    <path fill='none' stroke='currentColor' strokeLinecap='round' strokeLinejoin='round' strokeWidth='1.5' d='M7.023 5.5a9 9 0 1 0 9.953 0M12 2v8' color='currentColor'></path>
                  </svg>
                  Log Out
                </a>
              </li>

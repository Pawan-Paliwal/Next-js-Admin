...
const deleteOldImage = (imagePath) => {
  if (!imagePath) return;
  const fullPath = path.join(__dirname, '../uploads/OnlineImages/CollaborationImages', imagePath);
...
exports.getCollaborationById = (req, res) => {
  ...
  if (!CollaborationID) return res.status(400).json({ success: false, message: 'Missing ID' });
  const sql = 'SELECT * FROM mst_collaborationdata WHERE CollaborationID = ? LIMIT 1';
  db.query(sql, [CollaborationID], (err, results) => {
    if (err) return res.status(500).json({ success: false, message: 'Internal server error' });
    if (results.length === 0) return res.status(404).json({ success: false, message: 'Collaboration not found' });
...
exports.saveOrUpdateCollaboration = (req, res) => {
  ...
  const CollaborationImage = req?.files?.CollaborationImage?.[0]?.filename || null;
  ...
  `;
  const checkParams = CollaborationID ? [CollaborationName, CollaborationNameURL, CollaborationID] : [CollaborationName, CollaborationNameURL];

  db.query(checkDuplicateSql, checkParams, (err, results) => {
    if (err) return res.status(500).json({ success: false, message: "Database error", error: err });
    if (results.length > 0) {
      return res.status(400).json({ success: false, message: "Collaboration with the same name or URL already exists" });
    }

    if (CollaborationID) {
      const getOldSql = 'SELECT CollaborationImage FROM mst_collaborationdata WHERE CollaborationID = ?';
      db.query(getOldSql, [CollaborationID], (err, oldResults) => {
        ...
        const finalImage = CollaborationImage || oldResults[0].CollaborationImage;
        if (CollaborationImage && oldResults[0].CollaborationImage && CollaborationImage !== oldResults[0].CollaborationImage) {
        ...
        `;
        db.query(updateSql, [
          CollaborationName, CollaborationNameURL, finalImage, Description,
          DisplayOrder, ActiveStatus, UpdatedBy, currentTime, CollaborationID
        ], (err) => {
          if (err) return res.status(500).json({ success: false, message: "Update failed", error: err });
          return res.json({ success: true, message: "Collaboration updated successfully" });
        });
      });
    } else {
      const insertSql = `
      ...
      db.query(insertSql, [
        ...
        return res.json({ success: true, message: "Collaboration created successfully" });
      });
    }
  });
};
...
exports.updateDisplayOrder = (req, res) => {
  ...
  const queries = updates.map(item => {
    return new Promise((resolve, reject) => {
      db.query('UPDATE mst_collaborationdata SET DisplayOrder = ? WHERE CollaborationID = ?',
        [item.DisplayOrder, item.CollaborationID], (err, result) => {
...
exports.deleteCollaboration = (req, res) => {
  ...
  db.query(getImageSql, [CollaborationID], (err, results) => {
    ...
    db.query('DELETE FROM mst_collaborationdata WHERE CollaborationID = ?', [CollaborationID], (err, result) => {
      if (err) return res.status(500).json({ success: false, message: 'Database error' });
...
exports.updateActiveStatus = (req, res) => {
  ...
  if (!CollaborationID || ActiveStatus === undefined) return res.status(400).json({ success: false, message: "Missing ID/Status" });

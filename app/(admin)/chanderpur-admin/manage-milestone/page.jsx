...
import { useGetAllMilestonesQuery, useDeleteMilestoneMutation, useUpdateMilestoneStatusMutation, useUpdateDisplayOrderMutation } from "@/store/backendSlice/milestoneAPISlice";
...
export default function ManageMilestoneData() {
  ...
  const filteredData = milestones.filter((item) => {
    ...
    const matchesText = item.MilestoneName?.toLowerCase().includes(searchText) || item.MilestoneYear?.toLowerCase().includes(searchText);

...
import { useGetAllFacilityCategoriesQuery, useDeleteFacilityCategoryMutation, useUpdateFacilityCategoryStatusMutation, useUpdateFacilityCategoryDisplayOrderMutation } from "@/store/backendSlice/facilityCategoryAPISlice";
...
export default function ManageFacilityCategory() {
  ...
  const { data: categoryData = [], refetch, isLoading } = useGetAllFacilityCategoriesQuery();
  const categories = categoryData || [];
  const [deleteCategory] = useDeleteFacilityCategoryMutation();
  const [updateStatus] = useUpdateFacilityCategoryStatusMutation();
  const [updateDisplayOrder, { isLoading: isUpdatingOrder }] = useUpdateFacilityCategoryDisplayOrderMutation();
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    const confirmed = window.confirm("Are you sure you want to update the display order for all categories?");
    if (!confirmed) return;
  ...
  const columns = [
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            {row.CategoryImage ? (
              <img
                src={`/OnlineImages/FacilitycategoryImages/${row.CategoryImage}`}
    ...
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.CategoryID] ?? ""}
      ...
      ? [{
        ...
        cell: (row) =>
          isLoading ? (
            ...
            <Link
              href={`/chanderpur-admin/addupd-facilitycategory?ID=${row.CategoryID}`}
              className="edit-icon"
            >
    ...
    <div className="subheader-container">
      ...
      <div className="colB">
        ...
        {pagePermission?.CanAdd === 1 && (
          <Link href={"/chanderpur-admin/addupd-facilitycategory"} className="addnew-btn" style={{ width: "110px" }}>
            <span>+</span> Add New
          </Link>
        )}
      </div>
  ...
  return (
    <main>
      <DataTable
        title="Manage Facility Categories"
        columns={columns}
        data={filteredData}
        striped
        pagination

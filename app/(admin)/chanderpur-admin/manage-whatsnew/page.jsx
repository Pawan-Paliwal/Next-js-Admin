...
import { useGetAllWhatsNewQuery, useDeleteWhatsNewMutation, useUpdateWhatsNewStatusMutation, useUpdateWhatsNewDisplayOrderMutation } from "@/store/backendSlice/whatsNewAPISlice";
...
export default function ManageWhatsNewData() {
  ...
  const { data: whatsNewData = [], refetch, isLoading } = useGetAllWhatsNewQuery();
  const listItems = whatsNewData || [];
  const [deleteItem] = useDeleteWhatsNewMutation();
  const [updateStatus] = useUpdateWhatsNewStatusMutation();
  const [updateDisplayOrder, { isLoading: isUpdatingOrder }] = useUpdateWhatsNewDisplayOrderMutation();
  ...
  useEffect(() => {
    if (listItems.length > 0) {
      const initialOrders = {};
      listItems.forEach(s => {
        initialOrders[s.WhatsNewID] = s.DisplayOrder;
      });
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    const updates = Object.entries(displayOrders).map(([id, order]) => ({
      WhatsNewID: parseInt(id),
      DisplayOrder: parseInt(order) || 0,
    }));

    const confirmed = window.confirm("Are you sure you want to update the display order for all items?");
    if (!confirmed) return;
  ...
  const handleDelete = async (WhatsNewID) => {
    const confirmed = confirm("Are you sure you want to delete this item?");
    if (!confirmed) return;
    try {
      const res = await deleteItem(WhatsNewID).unwrap();
      if (res.success) {
  ...
  const handleApprove = async (WhatsNewID, currentStatus) => {
    if (pagePermission?.CanWrite !== 1) {
    ...
    try {
      ...
      const res = await updateStatus({ WhatsNewID, ActiveStatus: updatedStatus }).unwrap();
      if (res.success) {
  ...
  const columns = [
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            {row.WhatsNewImage ? (
              <img
                src={`/OnlineImages/WhatsnewImages/${row.WhatsNewImage}`}
                alt={row.WhatsNewName}
    ...
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.WhatsNewID] ?? ""}
          onChange={(e) => handleDisplayOrderChange(row.WhatsNewID, e.target.value)}
          ...
          <>
            ...
            {pagePermission?.CanWrite === 1 && (
              <button
                ...
                onClick={() => handleApprove(row.WhatsNewID, row.ActiveStatus)}
              >
                {row.ActiveStatus ? (
                  <svg
      ...
      ? [{
        ...
        cell: (row) =>
          isLoading ? (
            ...
            <Link
              href={`/chanderpur-admin/addupd-whatsnew?ID=${row.WhatsNewID}`}
              className="edit-icon"
            >
      ...
      ? [{
        ...
        cell: (row) =>
          isLoading ? (
            ...
            <button onClick={() => handleDelete(row.WhatsNewID)} className="edit-icon">
  ...
  const filteredData = listItems.filter((item) => {
    ...
    const matchesText = item.WhatsNewName?.toLowerCase().includes(searchText);
    ...
    <div className="subheader-container">
      ...
      <div className="colB">
        ...
        {pagePermission?.CanAdd === 1 && (
          <Link href={"/chanderpur-admin/addupd-whatsnew"} className="addnew-btn" style={{ width: "110px" }}>
            <span>+</span> Add New
          </Link>
        )}
      </div>
  ...
  return (
    <main>
      <DataTable
        title="Manage Whats New Section"
        columns={columns}
        data={filteredData}
        striped
        pagination

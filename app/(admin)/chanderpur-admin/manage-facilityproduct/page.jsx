...
import { useGetAllFacilityProductsQuery, useDeleteFacilityProductMutation, useUpdateFacilityProductStatusMutation, useUpdateFacilityProductDisplayOrderMutation } from "@/store/backendSlice/facilityProductAPISlice";
...
export default function ManageFacilityProduct() {
  ...
  const { data: productData = [], refetch, isLoading } = useGetAllFacilityProductsQuery();
  const products = productData || [];

  const [deleteProduct] = useDeleteFacilityProductMutation();
  const [updateStatus] = useUpdateFacilityProductStatusMutation();
  const [updateDisplayOrder, { isLoading: isUpdatingOrder }] = useUpdateFacilityProductDisplayOrderMutation();
  ...
  useEffect(() => {
    if (products.length > 0) {
      const initialOrders = {};
      products.forEach(s => {
        initialOrders[s.ProductID] = s.DisplayOrder;
      });
  ...
  const uniqueCategories = useMemo(() => {
    ...
    return Array.from(catsMap.entries()).map(([id, name]) => ({ CategoryID: id, CategoryName: name }));
  }, [products]);
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    const confirmed = window.confirm("Are you sure you want to update the display order for all products?");
    if (!confirmed) return;
  ...
  const columns = [
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            {row.FacilityDefaultImage ? (
              <img
                src={`/OnlineImages/FacilityproductImages/${row.FacilityDefaultImage}`}
                alt={row.ProductName}
    ...
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.ProductID] ?? ""}
          onChange={(e) => handleDisplayOrderChange(row.ProductID, e.target.value)}
      ...
      ? [{
        ...
        cell: (row) =>
          isLoading ? (
            ...
            <Link
              href={`/chanderpur-admin/addupd-facilityproduct?ID=${row.ProductID}`}
              className="edit-icon"
            >
  ...
  const filteredData = products.filter((item) => {
    ...
    return matchesText && matchesOption && matchesCategory;
  });
    ...
    <div className="subheader-container">
      ...
      <div className="colB">
        ...
        {pagePermission?.CanAdd === 1 && (
          <Link href={"/chanderpur-admin/addupd-facilityproduct"} className="addnew-btn" style={{ width: "110px" }}>
            <span>+</span> Add New
          </Link>
        )}
      </div>
  ...
  return (
    <main>
      <DataTable
        title="Manage Facility Products"
        columns={columns}
        data={filteredData}
        striped
        pagination

...
import { useGetAllCompaniesQuery, useDeleteCompanyMutation, useUpdateCompanyStatusMutation, useUpdateDisplayOrderMutation } from "@/store/backendSlice/companyAPISlice";
...
export default function ManageCompanyData() {
  ...
  const { data: companyData = [], refetch, isLoading } = useGetAllCompaniesQuery();
  ...
  useEffect(() => {
    if (Array.isArray(companies)) {
      const initialOrders = {};
      companies.forEach(s => {
        initialOrders[s.CompanyID] = s.DisplayOrder;
      });
  ...
  const handleDisplayOrderChange = (CompanyID, value) => {
    setDisplayOrders(prev => ({
      ...prev,
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    if (!confirmed) return;
    const data = Object.entries(displayOrders).map(([CompanyID, DisplayOrder]) => ({
      CompanyID,
      DisplayOrder
    }));
    try {
  ...
  const handleApprove = async (CompanyID, currentStatus) => {
    const confirmed = confirm("Are you sure you want to update this status?");
    if (!confirmed) return;

    try {
      const updatedStatus = currentStatus === 1 ? 0 : 1;
      const res = await updateStatus({ CompanyID, ActiveStatus: updatedStatus }).unwrap();
      if (res.success) {
  ...
  const columns = [
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            {row.CompanyImage ? (
              <img
                src={`/OnlineImages/CompanyImages/${row.CompanyImage}`}
                alt={row.CompanyName}
    ...
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.CompanyID] !== undefined ? displayOrders[row.CompanyID] : ""}
          onChange={(e) => handleDisplayOrderChange(row.CompanyID, e.target.value)}
        ...
        isLoading ? (
          ...
          <>
            ...
            <button
              ...
              onClick={() => handleApprove(row.CompanyID, row.ActiveStatus)}
            >
              {row.ActiveStatus ? (
                <svg
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <Link
            href={`/chanderpur-admin/addupd-company?ID=${row.CompanyID}`}
            className="edit-icon"
          >
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <button onClick={() => handleDelete(row.CompanyID)} className="edit-icon">
  ...
  const filteredData = companies.filter((item) => {
    ...
    const matchesText = item.CompanyName?.toLowerCase().includes(searchText);
  ...
  const subHeaderComponent = (
    <div className="subheader-container">
      ...
      <div className="colB">
        ...
        <Link href={"/chanderpur-admin/addupd-company"} className="addnew-btn" style={{ width: "110px" }}>
          <span>+</span> Add New
        </Link>
      </div>
    </div>

...
import { useGetAllDirectorsQuery, useDeleteDirectorMutation, useUpdateDirectorStatusMutation, useUpdateDisplayOrderMutation } from "@/store/backendSlice/directorAPISlice";
...
export default function ManageDirectorData() {
  ...
  const { data: directorData = [], refetch, isLoading } = useGetAllDirectorsQuery();
  ...
  const [updateStatus] = useUpdateDirectorStatusMutation();
  ...
  useEffect(() => {
    if (Array.isArray(directors)) {
      const initialOrders = {};
      directors.forEach(s => {
        initialOrders[s.DirectorID] = s.DisplayOrder;
      });
  ...
  const handleDelete = async (DirectorID) => {
    const confirmed = confirm("Are you sure you want to delete this Director post?");
    if (!confirmed) return;
    try {
      const res = await deleteDirector(DirectorID).unwrap();
      if (res.success) {
        toast.success("Director deleted successfully");
        refetch();
      } else {
        toast.error("Error deleting Director.");
      }
    } catch (error) {
      console.error(error);
  ...
  const handleDisplayOrderChange = (DirectorID, value) => {
    setDisplayOrders(prev => ({
      ...prev,
      [DirectorID]: value === "" ? "" : parseInt(value, 10)
    }));
  };
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    if (!confirmed) return;
    const data = Object.entries(displayOrders).map(([DirectorID, DisplayOrder]) => ({
      DirectorID,
      DisplayOrder
    }));
    try {
  ...
  const handleApprove = async (DirectorID, currentStatus) => {
    const confirmed = confirm("Are you sure you want to update this status?");
    if (!confirmed) return;

    try {
      const updatedStatus = currentStatus === 1 ? 0 : 1;
      const res = await updateStatus({ DirectorID, ActiveStatus: updatedStatus }).unwrap();
      if (res.success) {
  ...
  const columns = [
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            {row.DirectorImage ? (
              <img
                src={`/OnlineImages/DirectorImages/${row.DirectorImage}`}
                alt={row.DirectorName}
            ...
            <span>{row.DirectorName} <br /> <small style={{ color: "gray" }}>{row.DirectorDesignation}</small></span>
          </div>
        ),
      width: "68%",
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.DirectorID] !== undefined ? displayOrders[row.DirectorID] : ""}
          onChange={(e) => handleDisplayOrderChange(row.DirectorID, e.target.value)}
        ...
        isLoading ? (
          ...
          <>
            ...
            <button
              ...
              onClick={() => handleApprove(row.DirectorID, row.ActiveStatus)}
            >
              {row.ActiveStatus ? (
                <svg
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <Link
            href={`/chanderpur-admin/addupd-director?ID=${row.DirectorID}`}
            className="edit-icon"
          >
    ...
    },
    {
      ...
      cell: (row) =>
        isLoading ? (
          ...
          <button onClick={() => handleDelete(row.DirectorID)} className="edit-icon">
  ...
  const filteredData = directors.filter((item) => {
    ...
    const matchesText = item.DirectorName?.toLowerCase().includes(searchText);
    const matchesOption =
  ...
  const subHeaderComponent = (
    <div className="subheader-container">
      ...
      <div className="colB">
        ...
        <Link
          href={"/chanderpur-admin/addupd-director"}

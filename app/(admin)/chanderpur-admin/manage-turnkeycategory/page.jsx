...
import { useGetAllClientTypesQuery, useDeleteClientTypeMutation, useUpdateClientTypeStatusMutation, useUpdateClientTypeDisplayOrderMutation } from "@/store/backendSlice/clientTypeAPISlice";
...
export default function ManageClientType() {
  ...
  const { data: typesData = [], refetch, isLoading } = useGetAllClientTypesQuery();
  const [deleteType] = useDeleteClientTypeMutation();
  const [updateStatus] = useUpdateClientTypeStatusMutation();
  const [updateDisplayOrder, { isLoading: isUpdatingOrder }] = useUpdateClientTypeDisplayOrderMutation();
  ...
  useEffect(() => {
    if (typesData.length > 0) {
      const initialOrders = {};
      typesData.forEach(s => {
        initialOrders[s.ClientTypeID] = s.DisplayOrder;
      });
  ...
  const handleUpdateDisplayOrder = async () => {
    ...
    const updates = Object.entries(displayOrders).map(([id, order]) => ({
      ClientTypeID: parseInt(id),
      DisplayOrder: parseInt(order) || 0,
    }));

    const confirmed = window.confirm("Are you sure you want to update the display order?");
    if (!confirmed) return;
  ...
  const handleApprove = async (id, currentStatus) => {
    ...
    const confirmed = confirm("Update status?");
    if (!confirmed) return;
    try {
      const res = await updateStatus({ ClientTypeID: id, ActiveStatus: currentStatus === 1 ? 0 : 1 }).unwrap();
      if (res.success) {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <input
          type="number"
          value={displayOrders[row.ClientTypeID] ?? ""}
          onChange={(e) => handleDisplayOrderChange(row.ClientTypeID, e.target.value)}
          ...
          <>
            ...
            {pagePermission?.CanWrite === 1 && (
              <button
                ...
                onClick={() => handleApprove(row.ClientTypeID, row.ActiveStatus)}
              >
                {row.ActiveStatus ? (
      ...
      ? [
        {
          ...
          cell: (row) =>
            isLoading ? (
              ...
              <Link
                href={`/chanderpur-admin/assign-clientlogos?ClientTypeID=${row.ClientTypeID}`}
                className="manage-media-btn"
                style={{ border: "1px solid #f26522", color: "#f26522", background: "transparent", padding: "4px 12px", borderRadius: "4px", textDecoration: "none", display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", fontWeight: "600" }}
              >
                <span>+</span> Add Logos
              </Link>
            ),
        ...
        },
        {
          ...
          cell: (row) =>
            isLoading ? (
              ...
              <Link href={`/chanderpur-admin/addupd-turnkeycategory?ID=${row.ClientTypeID}`} className="edit-icon">
      ...
      ? [{
        ...
        cell: (row) =>
          isLoading ? (
            ...
            <button onClick={() => handleDelete(row.ClientTypeID)} className="edit-icon">
  ...
  const filteredData = typesData.filter((item) => {
    ...
    const matchesText = item.TypeName?.toLowerCase().includes(searchText);
    ...
    <div className="subheader-container">
      ...
      <div className="colB">
        ...
        {pagePermission?.CanAdd === 1 && (
          <Link href="/chanderpur-admin/addupd-turnkeycategory" className="addnew-btn" style={{ width: "110px" }}>
            <span>+</span> Add New
          </Link>
        )}
      </div>

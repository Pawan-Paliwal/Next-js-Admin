...
import { useGetManufacturingQuery, useUpdateManufacturingStatusMutation, useDeleteManufacturingMutation, useUpdateDisplayOrderMutation } from "@/store/backendSlice/manufacturingAPISlice";
...
export default function ManageManufacturingData() {
    ...
    const { data: manufacturingData = [], refetch, isLoading } = useGetManufacturingQuery();
    const [updateStatus] = useUpdateManufacturingStatusMutation();
    const [deleteManufacturing] = useDeleteManufacturingMutation();
    const [updateDisplayOrder, { isLoading: isUpdatingOrder }] = useUpdateDisplayOrderMutation();
    const [displayOrders, setDisplayOrders] = useState({});

    useEffect(() => {
        if (Array.isArray(manufacturingData)) {
            const initialOrders = {};
            manufacturingData.forEach(s => {
                initialOrders[s.ManufacturingID] = s.DisplayOrder;
            });
    ...
    const handleDelete = async (ManufacturingID) => {
        if (pagePermission?.CanDelete !== 1) {
            toast.error("You do not have permission to delete manufacturing");
            return;
        }
        const confirmed = confirm("Are you sure you want to delete this?");
        if (!confirmed) return;
        try {
            const res = await deleteManufacturing(ManufacturingID).unwrap();
            if (res.success) {
    ...
    const handleApprove = async (ManufacturingID, currentStatus) => {
        if (pagePermission?.CanWrite !== 1) {
        ...
        try {
            ...
            const res = await updateStatus({ ManufacturingID, ActiveStatus: updatedStatus }).unwrap();
            if (res.success) {
    ...
    const columns = [
        {
            ...
            selector: (row) => row.ManufacturingName,
            sortable: true,
            cell: (row) =>
                isLoading ? (
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column" }}>
                            <span style={{ fontWeight: "600" }}>{row.ManufacturingName}</span>
                        </div>
                    </div>
                ),
            width: "73%"
        },
        {
            ...
            cell: (row) => (
                <input
                    type="number"
                    value={displayOrders[row.ManufacturingID] ?? ""}
                    onChange={(e) => handleDisplayOrderChange(row.ManufacturingID, e.target.value)}
                    ...
                    <>
                        ...
                        {pagePermission?.CanWrite === 1 && (
                            <button
                                ...
                                onClick={() => handleApprove(row.ManufacturingID, row.ActiveStatus)}
                            >
                                {row.ActiveStatus ? (
                                    <svg
            ...
            ? [{
                ...
                cell: (row) =>
                    isLoading ? (
                        ...
                        <Link
                            href={`/chanderpur-admin/addupd-manufacturing?ID=${row.ManufacturingID}`}
                            className="edit-icon"
                        >
            ...
            ? [{
                ...
                cell: (row) =>
                    isLoading ? (
                        ...
                        <button onClick={() => handleDelete(row.ManufacturingID)} className="edit-icon">
    ...
    const filteredData = manufacturingData.filter((item) => {
        ...
        const matchesText = item.ManufacturingName?.toLowerCase().includes(searchText);
        ...
        <div className="subheader-container">
            ...
            <div className="colB">
                ...
                {pagePermission?.CanAdd === 1 && (
                    <Link href={"/chanderpur-admin/addupd-manufacturing"} className="addnew-btn" style={{ width: "110px" }}>
                        <span>+</span> Add New
                    </Link>
                )}
            </div>
